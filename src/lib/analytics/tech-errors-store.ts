...
function isNoiseRow(row: TechErrorRow): boolean {
  return isIgnorableClientError(row.message);
}
...
export async function upsertTechError(input: {
  ...
  file.errors = file.errors.filter(
    (e) => new Date(e.last_seen_at).getTime() >= cutoff && !isNoiseRow(e)
  );
...
export async function readTechErrors(): Promise<TechErrorRow[]> {
  const file = await readSafe();
  const kept = file.errors.filter((e) => !isNoiseRow(e));
  if (kept.length !== file.errors.length) {
    file.errors = kept;
    await mkdir(path.dirname(FILE), { recursive: true });
    await writeFile(FILE, JSON.stringify(file), "utf-8");
  }
